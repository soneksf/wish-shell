...
void Shell::run_builtin(const std::vector<std::string>& toks) {
    ...
    if (name == "cd") {
        if (toks.size() != 2) {  // рівно один аргумент
            print_error();
            return;
        }
        if (chdir(toks[1].c_str()) != 0) {  // каталогу немає або немає прав
            print_error();
        }
        return;
    }

    // path: 0 або більше аргументів, завжди перезаписує старий шлях
...
std::string Shell::resolve(const std::string& name) const {
    for (const auto& dir : path_) {
        ...
        full += name;
        if (access(full.c_str(), X_OK) == 0) {  // файл існує і виконуваний
            return full;
        }
    }
    return std::string();
...
void Shell::run_line(const std::string& line) {
    ...
    if (is_builtin(toks[0])) {  // вбудовані виконуються в самому шеллі
        run_builtin(toks);
        return;
    }

    const std::string exe = resolve(toks[0]);
    if (exe.empty()) {  // програми немає в жодному каталозі шляху
        print_error();
        return;
    }
